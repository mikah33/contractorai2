...
const PhotoUploadModal: React.FC<PhotoUploadModalProps> = ({ isOpen, onClose }) => {
  ...
  const borderColor = theme === 'light' ? 'border-gray-200' : 'border-theme/30';
      ...
      >
        ...
        <div className={`flex items-center justify-between px-5 pt-5 pb-4 border-b ${borderColor}`}>
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 bg-theme/20 rounded-xl flex items-center justify-center">
        ...
        <div className="p-4 overflow-y-auto max-h-[65vh]">
          ...
          {step === 'choose-destination' && (
            <div className="space-y-3">
              ...
              >
                <div className="w-12 h-12 bg-theme/20 rounded-xl flex items-center justify-center">
                  <FolderPlus className="w-6 h-6 text-theme" />
                </div>
                <div className="flex-1 text-left">
                ...
                <div className="mt-4 space-y-2">
                  ...
                  {projects.length === 0 ? (
                    <div className={`text-center py-6 ${bgCard} rounded-xl`}>
                      ...
                      <button
                        onClick={() => handleDestinationSelect('new-project')}
                        className="mt-3 text-theme text-sm font-medium"
                      >
              ...
              {destination === 'new-project' && (
                <div className="mt-4 space-y-3">
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 ${inputBg} border rounded-xl ${inputText} focus:ring-2 focus:ring-theme focus:border-transparent`}
                    autoFocus
                  />
                  <button
                    onClick={handleCreateProject}
                    disabled={!newProjectName.trim()}
                    className={`w-full py-3 bg-theme text-white rounded-xl font-semibold hover:bg-[#035291] disabled:${theme === 'light' ? 'bg-gray-300 text-gray-500' : 'bg-zinc-700 text-zinc-500'} active:scale-[0.98] transition-all`}
                  >
                    Create & Continue
                  </button>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-6 ${bgCard} border-2 border-dashed border-theme/50 rounded-xl hover:border-theme active:scale-[0.98] transition-all`}
                >
                  <Camera className="w-10 h-10 text-theme mb-2" />
                ...
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-6 ${bgCard} border-2 border-dashed border-theme/50 rounded-xl hover:border-theme active:scale-[0.98] transition-all`}
                >
                  <Image className="w-10 h-10 text-theme mb-2" />
              ...
              {photos.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => fileInputRef.current?.click()}
                      className="flex items-center gap-1 text-theme text-sm font-medium"
                    >
              ...
              {photos.length > 0 && (
                <button
                  ...
                  className={`w-full py-3.5 bg-theme text-white rounded-xl font-semibold flex items-center justify-center gap-2 hover:bg-[#035291] disabled:${theme === 'light' ? 'bg-gray-300' : 'bg-zinc-700'} active:scale-[0.98] transition-all`}
                >
                  {isUploading ? (
                    <>
