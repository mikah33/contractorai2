...
// Default lumber materials from FramingCalculator
const DEFAULT_LUMBER_MATERIALS = [
  { name: '2x4 Lumber', unit: 'per piece', price: 3.98, category: 'lumber', unitSpec: '8 ft' },
  { name: '2x6 Lumber', unit: 'per piece', price: 5.98, category: 'lumber', unitSpec: '8 ft' }
];

const DEFAULT_SHEATHING_MATERIALS = [
  { name: 'OSB 7/16"', unit: 'per 4x8 sheet', price: 15.98, category: 'sheathing', unitSpec: '32 sq ft' },
  { name: 'OSB 15/32"', unit: 'per 4x8 sheet', price: 18.98, category: 'sheathing', unitSpec: '32 sq ft' },
  { name: 'OSB 19/32"', unit: 'per 4x8 sheet', price: 22.98, category: 'sheathing', unitSpec: '32 sq ft' },
  { name: 'Plywood 7/16"', unit: 'per 4x8 sheet', price: 24.98, category: 'sheathing', unitSpec: '32 sq ft' },
  { name: 'Plywood 15/32"', unit: 'per 4x8 sheet', price: 28.98, category: 'sheathing', unitSpec: '32 sq ft' },
  { name: 'Plywood 19/32"', unit: 'per 4x8 sheet', price: 32.98, category: 'sheathing', unitSpec: '32 sq ft' }
];

const DEFAULT_HARDWARE_MATERIALS = [
  { name: 'Tiedown', unit: 'per piece', price: 12.98, category: 'hardware', unitSpec: '1 count' },
  { name: 'Framing Nails (Passlode 3" 1000ct box)', unit: 'per box', price: 89.98, category: 'hardware', unitSpec: '1000 count' }
];

const UNIT_OPTIONS = [
  { value: 'piece', label: 'per piece' },
  { value: 'sheet', label: 'per 4x8 sheet' },
  { value: 'box', label: 'per box' }
];

const ConfigureFramingCalculator = () => {
  const navigate = useNavigate();
  const [activeSection, setActiveSection] = useState<'lumber' | 'sheathing' | 'hardware'>('lumber');
  const [configId, setConfigId] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);

  // Default materials with editable state
  const [lumberDefaults, setLumberDefaults] = useState(DEFAULT_LUMBER_MATERIALS);
  const [sheathingDefaults, setSheathingDefaults] = useState(DEFAULT_SHEATHING_MATERIALS);
  const [hardwareDefaults, setHardwareDefaults] = useState(DEFAULT_HARDWARE_MATERIALS);

  // Custom materials (added by user)
  const [customLumber, setCustomLumber] = useState<CustomMaterial[]>([]);
  const [customSheathing, setCustomSheathing] = useState<CustomMaterial[]>([]);
  ...
  const loadConfiguration = async () => {
    setLoading(true);
    try {
      const configResult = await customCalculatorService.getOrCreateConfig('framing');
      ...
      if (config.is_configured) {
        ...
        if (materialsResult.success && materialsResult.data) {
          ...
          const lumber = materials.filter(m => m.category === 'lumber');
          const sheathing = materials.filter(m => m.category === 'sheathing');
          const hardware = materials.filter(m => m.category === 'hardware');

          // For simplicity, treat all loaded materials as custom for now
          setCustomLumber(lumber);
          setCustomSheathing(sheathing);
  ...
  const saveDefaultEdit = async (index: number, category: string) => {
    ...
    if (result.success) {
      ...
      const updater = category === 'lumber' ? setLumberDefaults :
                     category === 'sheathing' ? setSheathingDefaults :
                     setHardwareDefaults;
  ...
  const handleReset = async () => {
    ...
    if (result.success) {
      setLumberDefaults(DEFAULT_LUMBER_MATERIALS);
      setSheathingDefaults(DEFAULT_SHEATHING_MATERIALS);
  ...
  const getCurrentDefaults = () => {
    switch (activeSection) {
      case 'lumber': return lumberDefaults;
      case 'sheathing': return sheathingDefaults;
  ...
  const getCurrentCustom = () => {
    switch (activeSection) {
      case 'lumber': return customLumber;
      case 'sheathing': return customSheathing;
    ...
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Configure Framing Calculator</h1>
            <p className="text-gray-600 mt-1">Customize materials and pricing for your framing estimates</p>
          </div>
          <button
            onClick={handleReset}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="border-b border-gray-200">
          <nav className="flex gap-4 px-6">
            {(['lumber', 'sheathing', 'hardware'] as const).map((section) => (
              <button
                key={section}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={activeSection === 'lumber' ? 'e.g., 2x8 Lumber' :
                              activeSection === 'sheathing' ? 'e.g., OSB 23/32"' :
                              'e.g., Hurricane Ties'}
                />
              </div>
...
export default ConfigureFramingCalculator;
