...
// Default drywall materials
const DEFAULT_SHEETS_MATERIALS = [
  { name: '1/2" Drywall Sheet', unit: 'per sheet', price: 15.98, category: 'sheets', unitSpec: '4x8 ft' },
  { name: '5/8" Drywall Sheet', unit: 'per sheet', price: 17.98, category: 'sheets', unitSpec: '4x8 ft' }
];

const DEFAULT_FINISHING_MATERIALS = [
  { name: 'Joint Compound (Mud)', unit: 'per 5-gallon bucket', price: 19.98, category: 'finishing', unitSpec: '5 gallons' },
  { name: 'Paper Tape', unit: 'per roll', price: 4.98, category: 'finishing', unitSpec: '250 ft' },
  { name: 'Mesh Tape', unit: 'per roll', price: 6.98, category: 'finishing', unitSpec: '150 ft' }
];

const DEFAULT_FASTENERS_MATERIALS = [
  { name: 'Drywall Screws', unit: 'per box', price: 8.98, category: 'fasteners', unitSpec: '1 lb box' }
];

const UNIT_OPTIONS = [
  { value: 'sheet', label: 'per sheet' },
  { value: 'panel', label: 'per panel' },
  { value: 'bucket', label: 'per bucket' },
  { value: 'roll', label: 'per roll' },
  { value: 'box', label: 'per box' },
  { value: 'unit', label: 'per unit' }
];

const ConfigureDrywallCalculator = () => {
  const navigate = useNavigate();
  const [activeSection, setActiveSection] = useState<'sheets' | 'finishing' | 'fasteners'>('sheets');
  const [configId, setConfigId] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);

  // Default materials with editable state
  const [sheetsDefaults, setSheetsDefaults] = useState(DEFAULT_SHEETS_MATERIALS);
  const [finishingDefaults, setFinishingDefaults] = useState(DEFAULT_FINISHING_MATERIALS);
  const [fastenersDefaults, setFastenersDefaults] = useState(DEFAULT_FASTENERS_MATERIALS);

  // Custom materials (added by user)
  const [customSheets, setCustomSheets] = useState<CustomMaterial[]>([]);
  const [customFinishing, setCustomFinishing] = useState<CustomMaterial[]>([]);
  const [customFasteners, setCustomFasteners] = useState<CustomMaterial[]>([]);
  ...
  const [editUnit, setEditUnit] = useState('sheet');
  ...
  const [newUnit, setNewUnit] = useState('sheet');
  ...
  const loadConfiguration = async () => {
    setLoading(true);
    try {
      const configResult = await customCalculatorService.getOrCreateConfig('drywall');
      ...
      if (config.is_configured) {
        ...
        if (materialsResult.success && materialsResult.data) {
          ...
          const sheets = materials.filter(m => m.category === 'sheets');
          const finishing = materials.filter(m => m.category === 'finishing');
          const fasteners = materials.filter(m => m.category === 'fasteners');

          // For simplicity, treat all loaded materials as custom for now
          setCustomSheets(sheets);
          setCustomFinishing(finishing);
          setCustomFasteners(fasteners);
        }
      }
    } catch (error) {
  ...
  const saveDefaultEdit = async (index: number, category: string) => {
    ...
    if (result.success) {
      ...
      const updater = category === 'sheets' ? setSheetsDefaults :
                     category === 'finishing' ? setFinishingDefaults :
                     setFastenersDefaults;
  ...
  const saveCustomEdit = async () => {
    ...
    const currentMaterial = [...customSheets, ...customFinishing, ...customFasteners].find(m => m.id === editingCustomId);
  ...
  const handleReset = async () => {
    ...
    if (result.success) {
      setSheetsDefaults(DEFAULT_SHEETS_MATERIALS);
      setFinishingDefaults(DEFAULT_FINISHING_MATERIALS);
      setFastenersDefaults(DEFAULT_FASTENERS_MATERIALS);
      await loadConfiguration();
    } else {
  ...
  const getCurrentDefaults = () => {
    switch (activeSection) {
      case 'sheets': return sheetsDefaults;
      case 'finishing': return finishingDefaults;
      case 'fasteners': return fastenersDefaults;
    }
  };

  const getCurrentCustom = () => {
    switch (activeSection) {
      case 'sheets': return customSheets;
      case 'finishing': return customFinishing;
      case 'fasteners': return customFasteners;
    }
  };
    ...
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Configure Drywall Calculator</h1>
            <p className="text-gray-600 mt-1">Customize materials and pricing for your drywall installation estimates</p>
          </div>
          <button
            onClick={handleReset}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="border-b border-gray-200">
          <nav className="flex gap-4 px-6">
            {(['sheets', 'finishing', 'fasteners'] as const).map((section) => (
              <button
                key={section}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={activeSection === 'sheets' ? 'e.g., 1/4" Drywall Sheet' :
                              activeSection === 'finishing' ? 'e.g., Premium Joint Compound' :
                              'e.g., Drywall Nails'}
                />
              </div>
...
export default ConfigureDrywallCalculator;
