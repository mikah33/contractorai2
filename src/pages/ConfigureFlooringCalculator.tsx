...
// Default flooring materials from FlooringCalculator
const DEFAULT_FLOORING_MATERIALS = [
  { name: '3/4" Oak Strip', unit: 'per box (25 sq ft)', unitSpec: '', price: 159.98, category: 'flooring' },
  { name: '3/4" Oak Plank', unit: 'per box (25 sq ft)', unitSpec: '', price: 179.98, category: 'flooring' },
  { name: '3/8" Engineered Oak', unit: 'per box (22 sq ft)', unitSpec: '', price: 129.98, category: 'flooring' },
  { name: '1/2" Engineered Maple', unit: 'per box (20 sq ft)', unitSpec: '', price: 149.98, category: 'flooring' },
  { name: '8mm Laminate', unit: 'per box (22.5 sq ft)', unitSpec: '', price: 49.98, category: 'flooring' },
  { name: '12mm Laminate', unit: 'per box (17.5 sq ft)', unitSpec: '', price: 69.98, category: 'flooring' },
  { name: 'Luxury Vinyl Plank', unit: 'per box (23.64 sq ft)', unitSpec: '', price: 89.98, category: 'flooring' },
  { name: 'WPC Vinyl Plank', unit: 'per box (19.2 sq ft)', unitSpec: '', price: 109.98, category: 'flooring' },
  { name: 'Plush Carpet', unit: 'per box (144 sq ft)', unitSpec: '', price: 359.98, category: 'flooring' },
  { name: 'Berber Carpet', unit: 'per box (144 sq ft)', unitSpec: '', price: 299.98, category: 'flooring' }
];

const DEFAULT_UNDERLAYMENT_MATERIALS = [
  { name: 'Standard Foam', unit: 'per sqft', unitSpec: '', price: 0.45, category: 'underlayment' },
  { name: 'Premium Foam with Vapor Barrier', unit: 'per sqft', unitSpec: '', price: 0.75, category: 'underlayment' },
  { name: 'Moisture Barrier', unit: 'per sqft', unitSpec: '', price: 0.35, category: 'underlayment' }
];

const DEFAULT_TRIM_MATERIALS = [
  { name: 'Quarter Round', unit: 'per linear foot', unitSpec: '', price: 2.50, category: 'trim' },
  { name: 'Baseboards', unit: 'per linear foot', unitSpec: '', price: 3.50, category: 'trim' }
];

const UNIT_OPTIONS = [
  { value: 'box', label: 'per box' },
  { value: 'square_foot', label: 'per sqft' },
  { value: 'square_yard', label: 'per square yard' },
  { value: 'linear_foot', label: 'per linear foot' },
  { value: 'plank', label: 'per plank' },
  { value: 'tile', label: 'per tile' },
  { value: 'roll', label: 'per roll' },
  { value: 'unit', label: 'per unit' }
];

const ConfigureFlooringCalculator = () => {
  const navigate = useNavigate();
  const [activeSection, setActiveSection] = useState<'flooring' | 'underlayment' | 'trim'>('flooring');
  const [configId, setConfigId] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);

  // Default materials with editable state
  const [flooringDefaults, setFlooringDefaults] = useState(DEFAULT_FLOORING_MATERIALS);
  ...
  const [customFlooring, setCustomFlooring] = useState<CustomMaterial[]>([]);
  ...
  const [editUnit, setEditUnit] = useState('box');
  ...
  const loadConfiguration = async () => {
    setLoading(true);
    try {
      const configResult = await customCalculatorService.getOrCreateConfig('flooring');
      ...
      if (config.is_configured) {
        ...
        if (materialsResult.success && materialsResult.data) {
          ...
          const flooring = materials.filter(m => m.category === 'flooring');
  ...
  const saveDefaultEdit = async (index: number, category: string) => {
    ...
    if (result.success) {
      ...
      const updater = category === 'flooring' ? setFlooringDefaults :
  ...
  const handleReset = async () => {
    ...
    if (result.success) {
      setFlooringDefaults(DEFAULT_FLOORING_MATERIALS);
  ...
  const getCurrentDefaults = () => {
    switch (activeSection) {
      case 'flooring': return flooringDefaults;
  ...
  const getCurrentCustom = () => {
    switch (activeSection) {
      case 'flooring': return customFlooring;
    ...
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Configure Flooring Calculator</h1>
            <p className="text-gray-600 mt-1">Customize materials and pricing for your flooring estimates</p>
          </div>
          <button
            onClick={handleReset}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="border-b border-gray-200">
          <nav className="flex gap-4 px-6">
            {(['flooring', 'underlayment', 'trim'] as const).map((section) => (
              <button
                key={section}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={activeSection === 'flooring' ? 'e.g., Premium Hardwood' :
                              activeSection === 'underlayment' ? 'e.g., Premium Moisture Barrier' :
                              'e.g., Crown Molding'}
                />
              </div>
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Specify the quantity (e.g., "25 sq ft" for a box)</p>
              </div>
            </div>
...
export default ConfigureFlooringCalculator;
