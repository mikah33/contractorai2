...
// Default equipment materials from HVACCalculator (lines 204-219)
const DEFAULT_EQUIPMENT_MATERIALS = [
  { name: 'Condenser Unit', unit: 'per ton', price: 1200, category: 'equipment', unitSpec: '1-5 ton capacity' },
  { name: 'Air Handler', unit: 'per ton', price: 800, category: 'equipment', unitSpec: '1-5 ton capacity' },
  { name: 'Package Unit', unit: 'per ton', price: 1800, category: 'equipment', unitSpec: '2-5 ton capacity' },
  { name: 'Mini-Split Outdoor Unit', unit: 'per ton', price: 1500, category: 'equipment', unitSpec: '9000-36000 BTU' },
  { name: 'Mini-Split Indoor Head', unit: 'per head', price: 500, category: 'equipment', unitSpec: 'wall-mounted' },
  { name: 'Split System Installation Labor', unit: 'per ton', price: 1000, category: 'equipment', unitSpec: 'standard install' },
  { name: 'Package Unit Installation Labor', unit: 'per ton', price: 800, category: 'equipment', unitSpec: 'roof-top install' }
];

const DEFAULT_DUCTWORK_MATERIALS = [
  { name: '4-6" Duct', unit: 'per linear foot', price: 4.98, category: 'ductwork', unitSpec: 'rigid or flex' },
  { name: '7-8" Duct', unit: 'per linear foot', price: 6.98, category: 'ductwork', unitSpec: 'rigid or flex' },
  { name: '9-10" Duct', unit: 'per linear foot', price: 8.98, category: 'ductwork', unitSpec: 'rigid or flex' },
  { name: '12"+ Duct', unit: 'per linear foot', price: 12.98, category: 'ductwork', unitSpec: 'trunk line' },
  { name: 'Small Elbow (4-8")', unit: 'per piece', price: 8.98, category: 'ductwork', unitSpec: '90-degree' },
  { name: 'Large Elbow (9"+)', unit: 'per piece', price: 12.98, category: 'ductwork', unitSpec: '90-degree' },
  { name: 'Takeoff', unit: 'per piece', price: 6.98, category: 'ductwork', unitSpec: 'branch connection' }
];

const DEFAULT_LINESET_MATERIALS = [
  { name: 'Refrigerant Line Set', unit: 'per linear foot', price: 12.98, category: 'linesets', unitSpec: '1/4" + 3/8" copper' },
  { name: 'Condensate Drain Line', unit: 'per linear foot', price: 2.98, category: 'linesets', unitSpec: '3/4" PVC' }
];

const DEFAULT_ACCESSORY_MATERIALS = [
  { name: 'Whole House Humidifier', unit: 'per unit', price: 299.98, category: 'accessories', unitSpec: 'bypass or fan-powered' },
  { name: 'UV Light System', unit: 'per unit', price: 249.98, category: 'accessories', unitSpec: 'in-duct installation' },
  { name: 'Air Cleaner System', unit: 'per unit', price: 399.98, category: 'accessories', unitSpec: 'HEPA filter' },
  { name: 'Zone Control Board', unit: 'per system', price: 299.98, category: 'accessories', unitSpec: 'up to 8 zones' },
  { name: 'Zone Damper', unit: 'per zone', price: 89.98, category: 'accessories', unitSpec: 'motorized' },
  { name: 'Zone Thermostat', unit: 'per zone', price: 79.98, category: 'accessories', unitSpec: 'programmable' }
];
...
const ConfigureHVACCalculator = () => {
  const navigate = useNavigate();
  const [activeSection, setActiveSection] = useState<'equipment' | 'ductwork' | 'linesets' | 'accessories'>('equipment');
  ...
  const [ductworkDefaults, setDuctworkDefaults] = useState(DEFAULT_DUCTWORK_MATERIALS);
  const [linesetDefaults, setLinesetDefaults] = useState(DEFAULT_LINESET_MATERIALS);
  ...
  const [customDuctwork, setCustomDuctwork] = useState<CustomMaterial[]>([]);
  const [customLinesets, setCustomLinesets] = useState<CustomMaterial[]>([]);
  ...
  const loadConfiguration = async () => {
    setLoading(true);
    try {
      const configResult = await customCalculatorService.getOrCreateConfig('hvac');
      ...
      if (config.is_configured) {
        ...
        if (materialsResult.success && materialsResult.data) {
          ...
          const ductwork = materials.filter(m => m.category === 'ductwork');
          const linesets = materials.filter(m => m.category === 'linesets');
          ...
          setCustomDuctwork(ductwork);
          setCustomLinesets(linesets);
  ...
  const saveDefaultEdit = async (index: number, category: string) => {
    ...
    if (result.success) {
      ...
      const updater = category === 'equipment' ? setEquipmentDefaults :
                     category === 'ductwork' ? setDuctworkDefaults :
                     category === 'linesets' ? setLinesetDefaults :
  ...
  const handleReset = async () => {
    ...
    if (result.success) {
      ...
      setDuctworkDefaults(DEFAULT_DUCTWORK_MATERIALS);
      setLinesetDefaults(DEFAULT_LINESET_MATERIALS);
  ...
  const getCurrentDefaults = () => {
    switch (activeSection) {
      ...
      case 'ductwork': return ductworkDefaults;
      case 'linesets': return linesetDefaults;
  ...
  const getCurrentCustom = () => {
    switch (activeSection) {
      ...
      case 'ductwork': return customDuctwork;
      case 'linesets': return customLinesets;
    ...
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Configure HVAC Calculator</h1>
            <p className="text-gray-600 mt-1">Customize materials and pricing for your HVAC installation estimates</p>
          </div>
          <button
            onClick={handleReset}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="border-b border-gray-200">
          <nav className="flex gap-4 px-6">
            {(['equipment', 'ductwork', 'linesets', 'accessories'] as const).map((section) => (
              <button
                key={section}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={activeSection === 'equipment' ? 'e.g., High-Efficiency Heat Pump' :
                              activeSection === 'ductwork' ? 'e.g., Insulated Flex Duct' :
                              activeSection === 'linesets' ? 'e.g., Copper Line Set Kit' :
...
export default ConfigureHVACCalculator;
