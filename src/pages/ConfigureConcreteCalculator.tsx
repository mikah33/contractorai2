...
// Default concrete materials from ConcreteCalculator
const DEFAULT_CONCRETE_MATERIALS = [
  { name: '3000 PSI Concrete', unit: 'per cubic yard', price: 125, category: 'concrete', unitSpec: '1 cubic yard' },
  { name: '4000 PSI Concrete', unit: 'per cubic yard', price: 145, category: 'concrete', unitSpec: '1 cubic yard' },
  { name: '5000 PSI Concrete', unit: 'per cubic yard', price: 165, category: 'concrete', unitSpec: '1 cubic yard' }
];

const DEFAULT_FINISHING_MATERIALS = [
  { name: 'Broom Finish', unit: 'per square foot', price: 0.50, category: 'finishing', unitSpec: '100 sq ft' },
  { name: 'Smooth Trowel Finish', unit: 'per square foot', price: 0.75, category: 'finishing', unitSpec: '100 sq ft' },
  { name: 'Stamped Finish', unit: 'per square foot', price: 3.50, category: 'finishing', unitSpec: '100 sq ft' },
  { name: 'Exposed Aggregate', unit: 'per square foot', price: 4.25, category: 'finishing', unitSpec: '100 sq ft' }
];

const DEFAULT_REINFORCEMENT_MATERIALS = [
  { name: 'Wire Mesh', unit: 'per square foot', price: 0.35, category: 'reinforcement', unitSpec: '100 sq ft' },
  { name: '#4 Rebar', unit: 'per linear foot', price: 0.85, category: 'reinforcement', unitSpec: '100 linear ft' },
  { name: 'Fiber Reinforcement', unit: 'per cubic yard', price: 35, category: 'reinforcement', unitSpec: '1 cubic yard' }
];
...
const ConfigureConcreteCalculator = () => {
  const navigate = useNavigate();
  const [activeSection, setActiveSection] = useState<'concrete' | 'finishing' | 'reinforcement'>('concrete');
  ...
  const loadConfiguration = async () => {
    setLoading(true);
    try {
      const configResult = await customCalculatorService.getOrCreateConfig('concrete');
  ...
  const saveDefaultEdit = async (index: number, category: string) => {
    ...
    if (result.success) {
      ...
      const updater = category === 'concrete' ? setConcreteDefaults :
    ...
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Configure Concrete Calculator</h1>
            <p className="text-gray-600 mt-1">Customize materials and pricing for your concrete estimates</p>
          </div>
          <button
            onClick={handleReset}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="border-b border-gray-200">
          <nav className="flex gap-4 px-6">
            {(['concrete', 'finishing', 'reinforcement'] as const).map((section) => (
              <button
                key={section}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={activeSection === 'concrete' ? 'e.g., 6000 PSI High-Strength Concrete' :
                              activeSection === 'finishing' ? 'e.g., Polished Finish' :
                              'e.g., Steel Rebar #5'}
                />
              </div>
...
export default ConfigureConcreteCalculator;
