...
// Default door materials from DoorsWindowsCalculator
const DEFAULT_DOOR_MATERIALS = [
  { name: 'Entry Door - Steel', unit: 'per unit', unitSpec: '', price: 299.98, category: 'doors' },
  { name: 'Entry Door - Fiberglass', unit: 'per unit', unitSpec: '', price: 499.98, category: 'doors' },
  { name: 'Entry Door - Wood', unit: 'per unit', unitSpec: '', price: 699.98, category: 'doors' },
  { name: 'Interior Door - Hollow-Core', unit: 'per unit', unitSpec: '', price: 49.98, category: 'doors' },
  { name: 'Interior Door - Solid-Core', unit: 'per unit', unitSpec: '', price: 99.98, category: 'doors' },
  { name: 'Interior Door - Wood', unit: 'per unit', unitSpec: '', price: 199.98, category: 'doors' },
  { name: 'Patio Door - Vinyl', unit: 'per unit', unitSpec: '', price: 599.98, category: 'doors' },
  { name: 'Patio Door - Aluminum', unit: 'per unit', unitSpec: '', price: 799.98, category: 'doors' },
  { name: 'Patio Door - Wood', unit: 'per unit', unitSpec: '', price: 1299.98, category: 'doors' },
  { name: 'French Door - Wood', unit: 'per unit', unitSpec: '', price: 899.98, category: 'doors' },
  { name: 'French Door - Fiberglass', unit: 'per unit', unitSpec: '', price: 799.98, category: 'doors' },
  { name: 'French Door - Steel', unit: 'per unit', unitSpec: '', price: 699.98, category: 'doors' },
  { name: 'Bifold Door - Hollow-Core', unit: 'per unit', unitSpec: '', price: 99.98, category: 'doors' },
  { name: 'Bifold Door - Solid-Core', unit: 'per unit', unitSpec: '', price: 149.98, category: 'doors' },
  { name: 'Bifold Door - Wood', unit: 'per unit', unitSpec: '', price: 249.98, category: 'doors' }
];

const DEFAULT_WINDOW_MATERIALS = [
  { name: 'Single-Hung Window - Vinyl', unit: 'per unit', unitSpec: '', price: 199.98, category: 'windows' },
  ...
  { name: 'Double-Hung Window - Wood', unit: 'per unit', unitSpec: '', price: 499.98, category: 'windows' },
  ...
  { name: 'Sliding Window - Wood', unit: 'per unit', unitSpec: '', price: 499.98, category: 'windows' },
...
const DEFAULT_TRIM_MATERIALS = [
  { name: 'Basic Trim', unit: 'per linear foot', unitSpec: '', price: 2.98, category: 'trim' },
  { name: 'Colonial Trim', unit: 'per linear foot', unitSpec: '', price: 3.98, category: 'trim' },
  { name: 'Craftsman Trim', unit: 'per linear foot', unitSpec: '', price: 4.98, category: 'trim' },
  { name: 'Modern Trim', unit: 'per linear foot', unitSpec: '', price: 3.98, category: 'trim' }
];

const DEFAULT_HARDWARE_MATERIALS = [
  { name: 'Interior Door Hardware - Basic', unit: 'per set', unitSpec: '', price: 24.98, category: 'hardware' },
...
const DEFAULT_INSTALLATION_MATERIALS = [
  { name: 'Exterior Pre-Hung Frame Kit', unit: 'per kit', unitSpec: '', price: 149.98, category: 'installation' },
  { name: 'Interior Pre-Hung Frame Kit', unit: 'per kit', unitSpec: '', price: 79.98, category: 'installation' },
  { name: 'Insulation', unit: 'per 20ft roll', unitSpec: '', price: 12.98, category: 'installation' },
  { name: 'Flashing Tape', unit: 'per roll', unitSpec: '', price: 12.98, category: 'installation' },
  { name: 'Caulk', unit: 'per tube', unitSpec: '', price: 6.98, category: 'installation' },
  { name: 'Shim Packs', unit: 'per pack', unitSpec: '', price: 4.98, category: 'installation' }
];
...
const UNIT_OPTIONS = [
  ...
  { value: 'roll_20ft', label: 'per 20ft roll' },
  { value: 'tube', label: 'per tube' },
  { value: 'pack', label: 'per pack' }
];

const ConfigureDoorsWindowsCalculator = () => {
  const navigate = useNavigate();
  const [activeSection, setActiveSection] = useState<'doors' | 'windows' | 'trim' | 'hardware' | 'installation'>('doors');
  const [configId, setConfigId] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);

  // Default materials with editable state
  const [doorDefaults, setDoorDefaults] = useState(DEFAULT_DOOR_MATERIALS);
  const [windowDefaults, setWindowDefaults] = useState(DEFAULT_WINDOW_MATERIALS);
  const [trimDefaults, setTrimDefaults] = useState(DEFAULT_TRIM_MATERIALS);
  const [hardwareDefaults, setHardwareDefaults] = useState(DEFAULT_HARDWARE_MATERIALS);
  const [installationDefaults, setInstallationDefaults] = useState(DEFAULT_INSTALLATION_MATERIALS);

  // Custom materials (added by user)
  const [customDoors, setCustomDoors] = useState<CustomMaterial[]>([]);
  const [customWindows, setCustomWindows] = useState<CustomMaterial[]>([]);
  const [customTrim, setCustomTrim] = useState<CustomMaterial[]>([]);
  const [customHardware, setCustomHardware] = useState<CustomMaterial[]>([]);
  const [customInstallation, setCustomInstallation] = useState<CustomMaterial[]>([]);
  ...
  const loadConfiguration = async () => {
    setLoading(true);
    try {
      const configResult = await customCalculatorService.getOrCreateConfig('doors_windows');
      ...
      if (config.is_configured) {
        ...
        if (materialsResult.success && materialsResult.data) {
          ...
          const windows = materials.filter(m => m.category === 'windows');
          const trim = materials.filter(m => m.category === 'trim');
          const hardware = materials.filter(m => m.category === 'hardware');
          const installation = materials.filter(m => m.category === 'installation');

          // For simplicity, treat all loaded materials as custom for now
          setCustomDoors(doors);
          setCustomWindows(windows);
          setCustomTrim(trim);
          setCustomHardware(hardware);
          setCustomInstallation(installation);
        }
      }
    } catch (error) {
  ...
  const saveDefaultEdit = async (index: number, category: string) => {
    ...
    if (result.success) {
      ...
      const updater = category === 'doors' ? setDoorDefaults :
                     category === 'windows' ? setWindowDefaults :
                     category === 'trim' ? setTrimDefaults :
                     category === 'hardware' ? setHardwareDefaults :
  ...
  const saveCustomEdit = async () => {
    ...
    const result = await customCalculatorService.updateMaterial(editingCustomId, {
      ...
      price: editPrice,
      unit: editUnit,
      metadata: { unitSpec: editUnitSpec }
    });
  ...
  const handleReset = async () => {
    ...
    if (result.success) {
      setDoorDefaults(DEFAULT_DOOR_MATERIALS);
      setWindowDefaults(DEFAULT_WINDOW_MATERIALS);
      setTrimDefaults(DEFAULT_TRIM_MATERIALS);
      setHardwareDefaults(DEFAULT_HARDWARE_MATERIALS);
      setInstallationDefaults(DEFAULT_INSTALLATION_MATERIALS);
      await loadConfiguration();
    } else {
  ...
  const getUnitLabel = (value: string, spec?: string): string => {
    ...
    const baseLabel = option?.label || 'per unit';
  ...
  const getCurrentDefaults = () => {
    switch (activeSection) {
      case 'doors': return doorDefaults;
      case 'windows': return windowDefaults;
      case 'trim': return trimDefaults;
      case 'hardware': return hardwareDefaults;
      case 'installation': return installationDefaults;
    }
  };

  const getCurrentCustom = () => {
    switch (activeSection) {
      case 'doors': return customDoors;
      case 'windows': return customWindows;
      case 'trim': return customTrim;
      case 'hardware': return customHardware;
      case 'installation': return customInstallation;
    }
  };
    ...
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Configure Doors & Windows Calculator</h1>
            <p className="text-gray-600 mt-1">Customize materials and pricing for your door and window estimates</p>
          </div>
          <button
            onClick={handleReset}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="border-b border-gray-200">
          <nav className="flex gap-4 px-6">
            {(['doors', 'windows', 'trim', 'hardware', 'installation'] as const).map((section) => (
              <button
                key={section}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={activeSection === 'doors' ? 'e.g., Premium Entry Door - Mahogany' :
                              activeSection === 'windows' ? 'e.g., Bay Window - Triple Pane' :
                              activeSection === 'trim' ? 'e.g., Crown Molding' :
                              activeSection === 'hardware' ? 'e.g., Smart Lock Set' :
                              'e.g., Weatherstripping Kit'}
                />
              </div>
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Specify the quantity for this material</p>
              </div>
            </div>
...
export default ConfigureDoorsWindowsCalculator;
